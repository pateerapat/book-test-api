...
module.exports = {
    ...
    testPointIncrementer: async (query) => {
        try {
            let response = await connect().then(async (mongoose) => {
                try {
                    let result = await userSchema.updateOne({ id: query.id }, { $inc: { user_point: query.point } });
